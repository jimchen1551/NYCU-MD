    ...
    for j in parameter[:3]:
        for k in parameter[3][0][0]:
            for l in range(3):
                ...
                model_0_0.add(Dense(2, kernel_regularizer="l2", activation=i[1]))
                model_0_0.compile(loss="categorical_crossentropy", optimizer=i[2], metrics=["accuracy"])
                history_0_0 = model_0_0.fit(signal[1], label_0[1], validation_data=(signal[2], label_0[2]), epochs=300, batch_size=64, verbose=False)

                with open('model_0.txt', 'a') as f:
                    f.write('model_0_0\t'+function.index(i)+'\t'+parameter.index(j)+'\t'+k+'\t'+str(l)+'\t')
                    loss, accuracy = model_0_0.evaluate(signal[1], label_0[1], verbose=False)
                    f.write("Training accuracy = {:.2f}".format(accuracy)+'\t')
                    loss, accuracy = model_0_0.evaluate(signal[0], label_0[0], verbose=False)
                ...
                plt.title('model_0_0\t'+function.index(i)+'\t'+parameter.index(j)+'\t'+k+'\t'+str(l))
                plt.xlabel("Epochs")
                plt.ylabel("Loss")
                plt.legend()
                plt.savefig('model_0_0/'+function.index(i)+'_'+parameter.index(j)+'_'+k+'_'+str(l)+'_loss')
                plt.clf()
                ...
                plt.ylabel("Accuracy")
                plt.legend()
                plt.savefig('model_0_0/'+function.index(i)+'_'+parameter.index(j)+'_'+k+'_'+str(l)+'_accuracy')
                plt.clf()
    ...
    for j in parameter[:3]:
        for k in parameter[3][0][0]:
            for l in range(3):
                ...
                model_0_1.add(Dense(2, kernel_regularizer="l2", activation=i[1]))
                model_0_1.compile(loss="categorical_crossentropy", optimizer=i[2], metrics=["accuracy"])
                history_0_1 = model_0_1.fit(signal[1], label_0[1], validation_data=(signal[2], label_0[2]), epochs=300, batch_size=64, verbose=False)

                with open('model_0.txt', 'a') as f:
                    f.write('model_0_1\t'+function.index(i)+'\t'+parameter.index(j)+'\t'+k+'\t'+str(l)+'\t')
                    loss, accuracy = model_0_1.evaluate(signal[1], label_0[1], verbose=False)
                    f.write("Training accuracy = {:.2f}".format(accuracy)+'\t')
                    loss, accuracy = model_0_1.evaluate(signal[0], label_0[0], verbose=False)
                    f.write("Testing accuracy = {:.2f}".format(accuracy)+'\n')
                ...
                plt.plot(epochs, loss, "bo-", label="Training Loss")
                plt.plot(epochs, val_loss, "ro--", label="Validation Loss")
                plt.title('model_0_1\t'+function.index(i)+'\t'+parameter.index(j)+'\t'+k+'\t'+str(l))
                plt.xlabel("Epochs")
                plt.ylabel("Loss")
                plt.legend()
                plt.savefig('model_0_1/'+function.index(i)+'_'+parameter.index(j)+'_'+k+'_'+str(l)+'_loss')
                plt.clf()
                ...
                plt.ylabel("Accuracy")
                plt.legend()
                plt.savefig('model_0_1/'+function.index(i)+'_'+parameter.index(j)+'_'+k+'_'+str(l)+'_accuracy')
                plt.clf()
    ...
    for j in parameter[:3]:
        for k in parameter[3][0][0]:
            for l in range(3):
                ...
                model_0_2.add(Dense(2, kernel_regularizer="l2", activation=i[1]))
                model_0_2.compile(loss="categorical_crossentropy", optimizer=i[2], metrics=["accuracy"])
                history_0_2 = model_0_2.fit(signal[1], label_0[1], validation_data=(signal[2], label_0[2]), epochs=300, batch_size=64, verbose=False)


                with open('model_0.txt', 'a') as f:
                    f.write('model_0_2\t'+function.index(i)+'\t'+parameter.index(j)+'\t'+k+'\t'+str(l)+'\t')
                    loss, accuracy = model_0_2.evaluate(signal[1], label_0[1], verbose=False)
                    f.write("Training accuracy = {:.2f}".format(accuracy)+'\t')
                    loss, accuracy = model_0_2.evaluate(signal[0], label_0[0], verbose=False)
                    f.write("Testing accuracy = {:.2f}".format(accuracy)+'\n')
                ...
                plt.plot(epochs, loss, "bo-", label="Training Loss")
                plt.plot(epochs, val_loss, "ro--", label="Validation Loss")
                plt.title('model_0_2\t'+function.index(i)+'\t'+parameter.index(j)+'\t'+k+'\t'+str(l))
                plt.xlabel("Epochs")
                plt.ylabel("Loss")
                plt.legend()
                plt.savefig('model_0_2/'+function.index(i)+'_'+parameter.index(j)+'_'+k+'_'+str(l)+'_loss')
                plt.clf()
                ...
                plt.ylabel("Accuracy")
                plt.legend()
                plt.savefig('model_0_2/'+function.index(i)+'_'+parameter.index(j)+'_'+k+'_'+str(l)+'_accuracy')
                plt.clf()